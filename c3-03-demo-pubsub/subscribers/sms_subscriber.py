...
def on_message(channel, method, properties, body):
    try:
        ...
        print(f"\n[{datetime.now().strftime('%H:%M:%S')}] [SMS] {routing_key}")
        
        if routing_key == 'order.created':
            print(f"  Sending SMS: Order #{event_data.get('order_id')} received")
        elif routing_key == 'payment.failed':
            print(f"  Sending SMS: Payment failed for order #{event_data.get('order_id')}")
...
def main():
    ...
    print("Subscribed to: order.*, payment.*")
